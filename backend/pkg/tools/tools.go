...
func (fte *flowToolsExecutor) GetPentesterExecutor(cfg PentesterExecutorConfig) (ContextToolsExecutor, error) {
	...
	if upclaw.IsAvailable() {
		ce.definitions = append(ce.definitions, registryDefinitions[UpclawToolName])
		ce.handlers[UpclawToolName] = upclaw.Handle
	}
