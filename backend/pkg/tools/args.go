...
// UpclawAction is the argument schema for the UpClaw tool ("upclaw").
// UpClaw is the vendored AI-driven penetration testing CLI (single-file,
// zero-dependency Python) executed inside the flow's primary terminal
// container. See upclaw.go for the execution details.
type UpclawAction struct {
	Mode string `json:"mode" jsonschema:"required,type=string,enum=scan,enum=recon,enum=req,enum=cmp,enum=doctor,enum=tools" jsonschema_description:"UpClaw mode: 'scan' runs the full autonomous scan (recon + 31 built-in security checks + installed external tools) and returns a normalized findings summary; 'recon' runs information gathering only (fingerprint/ports/dns/paths); 'req' replays a single crafted HTTP request (Repeater); 'cmp' compares two HTTP responses (Comparer); 'doctor' runs the UpClaw environment self-check; 'tools' lists detected external tools inside the container"`
	Target string `json:"target" jsonschema:"type=string" jsonschema_description:"Mandatory for scan/recon/req modes: target host, URL or 'host:port', e.g. example.com, http://10.0.0.5/dvwa/login.php. The target MUST be covered by the authorization file scope"`
	AuthFile string `json:"auth_file" jsonschema:"required,type=string" jsonschema_description:"Absolute path (inside the container) to the UpClaw authorization JSON file with fields: authorized_by, scope (list of authorized hosts/CIDRs), optional valid_until and reference. UpClaw refuses to run scans without it — this is the compliance gate and cannot be skipped. Prepare it with the file tool if it does not exist yet and you have written authorization for the target"`
	TargetB string `json:"target_b" jsonschema:"type=string" jsonschema_description:"Second URL for cmp mode (the variant request, e.g. with an injected payload)"`
	Method string `json:"method" jsonschema:"type=string" jsonschema_description:"HTTP method for req mode (default GET)"`
	Headers Strings `json:"headers" jsonschema:"type=array" jsonschema_description:"Extra request headers for req mode, each as 'Header: value', e.g. 'Cookie: security=low'"`
	Data string `json:"data" jsonschema:"type=string" jsonschema_description:"Request body for req mode (POST data)"`
	Match Strings `json:"match" jsonschema:"type=array" jsonschema_description:"Response-body keywords for req mode to confirm a vulnerability triggered, e.g. 'SQL syntax'"`
	Checks string `json:"checks" jsonschema:"type=string" jsonschema_description:"Comma-separated modules for scan mode to enable only a subset, e.g. 'sqli,xss,dir,cors'. Default: all built-in modules"`
	NoExt Bool `json:"no_ext" jsonschema:"type=boolean" jsonschema_description:"Set true to disable the external-tools adapter phase in scan mode (only built-in zero-dependency checks run)"`
	ExtTools string `json:"ext_tools" jsonschema:"type=string" jsonschema_description:"Comma-separated external tools for scan mode, e.g. 'nuclei,sqlmap'. Default: all detected ones"`
	NucleiTags string `json:"nuclei_tags" jsonschema:"type=string" jsonschema_description:"Directional Nuclei tags for scan mode, e.g. 'cve,wordpress'"`
	NucleiSeverity string `json:"nuclei_severity" jsonschema:"type=string" jsonschema_description:"Minimal Nuclei severity filter for scan mode, e.g. 'high,critical'"`
	Ports string `json:"ports" jsonschema:"type=string" jsonschema_description:"Port list for scan/recon mode, e.g. '80,443' or '1-1000'"`
	SkipPorts Bool `json:"skip_ports" jsonschema:"type=boolean" jsonschema_description:"Set true to skip the port-scan phase"`
	Verbose Bool `json:"verbose" jsonschema:"type=boolean" jsonschema_description:"Set true for verbose UpClaw output"`
	Timeout Int64 `json:"timeout" jsonschema:"required,type=integer" jsonschema_description:"Execution time limit in seconds (UpClaw scans can run long: a full scan typically needs 600-3600; a recon 60-600). Use 0 for the server default. The tool returns a findings summary and report paths on completion"`
	Message string `json:"message" jsonschema:"required,type=string" jsonschema_description:"Engagement-log entry — a 1-2 short sentence running commentary explaining what you intend to achieve by running UpClaw. Written in the engagement language declared by your system prompt"`
}
