...
var toolsTypeMapping = map[string]ToolType{
	...
	UpclawToolName:             EnvironmentToolType,
...
var registryDefinitions = map[string]llms.FunctionDefinition{
	...
	UpclawToolName: {
		Name: UpclawToolName,
		Description: "Runs UpClaw, the vendored AI-driven penetration testing CLI, inside the flow's primary terminal container. " +
			"UpClaw performs evidence-based, hallucination-resistant web security testing: autonomous scan (31 built-in zero-dependency checks + auto-driven external tools like nuclei/nmap/sqlmap), recon, single-request replay (Repeater) and response comparison (Comparer). " +
			"Every scan requires an authorization JSON file (fields: authorized_by, scope, optional valid_until, reference) — UpClaw refuses targets outside the authorized scope, this compliance gate cannot be bypassed. " +
			"Use 'scan' as the primary wide-coverage sweep, then use 'terminal'/'req'/'cmp' for targeted exploitation of verified findings. " +
			"The result contains severity counts, verified findings with evidence, and paths to report.html/findings.json/trace.json inside the container for deeper analysis with the file tool",
		Parameters: reflector.Reflect(&UpclawAction{}),
	},
